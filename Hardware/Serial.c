...
{
	//??????
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE);//??GPIO??
	...
	USART_InitStructure.USART_Parity = USART_Parity_No;//???:No???(v)  Odd???  Even???
	USART_InitStructure.USART_StopBits = USART_StopBits_1;//???:1????
	USART_InitStructure.USART_WordLength = USART_WordLength_8b;//??:8?(??????????)
	USART_Init(USART2, &USART_InitStructure);
	
	USART_ITConfig(USART2,USART_IT_RXNE,ENABLE);//??RXNE????NVIC???
...
//	NVIC_Init(&NVIC_InitStructure);
	...
	USART_Cmd(USART2, ENABLE);//串口号填错，会导致I2C，串口都不显示
}
